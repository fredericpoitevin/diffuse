...
class BraggMasker:
    ...
    of (frames, flattened detector) that indicates pixels predicted to be spanned by Bragg peaks. """
    ...
    def _predict_phi(self, bins):
        ...
        for batch in range(36):
            ...
            # compute unrotated q vectors, pstar0
            ...
            lower, upper = 5*batch + 0.25, 5*batch + 5.25
            if batch == 0:
                lower = -5.25
            if batch == 35:
                upper = 185.25
    ...
    def predict_s1(self):
        ...
        lower, upper = 5*self.nbatch + 0.25, 5*self.nbatch + 5.25
        if self.nbatch == 0:
            lower = -5.25
        if self.nbatch == 35:
            upper = 185.25
    ...
    def _px_to_sdash(self):
        ...
        # convert pixel coordinates into mm in laboratory frame
        ...
        # calculate q vectors and normalize by wavelength
    ...
    def _save_masks(self, rs_mask):
        """ Save rs_masks in h5 format. (Too large as a single np.array.) """
        for i, item in enumerate(rs_mask):
            name = 'arr_%s' % i
            data = {name : item}
            io.saveh(self.system['savename'], **data)
        return
    ...
    def pred_pixels(self, sigma_n):
        ...
        rs_mask = np.zeros((360, 6224001))
        dtc_shape = self.system['shape']
        ...
        dpsi = np.tile(np.arange(0.5, 180.5, 0.5), (s1_batch.shape[0], 1)) - np.tile(self.predicted[:,3], (360, 1)).T
        ...
        for idx in np.unique(frames[0])[:10]:
            #eps1 = 180*np.inner(e1[idx], sdash - s1_batch[idx])/(np.pi*np.linalg.norm(s1_batch[idx]))
            #eps2 = 180*np.inner(e2[idx], sdash - s1_batch[idx])/(np.pi*np.linalg.norm(s1_batch[idx]))
            ...
            x_obs = np.where(np.abs(eps1) < delta_d)
            y_obs = np.where(np.abs(eps2) < delta_d)
            xy_obs = np.intersect1d(x_obs, y_obs)

            if len(xy_obs) > 0:
                images = np.array(frames[1][np.where(frames[0]==idx)[0]])
                rs_mask[images[0]:images[-1]+1, xy_obs] = 1

                xcen, ycen = int(np.median(xy_obs/dtc_shape[1])), int(np.median(xy_obs % dtc_shape[1]))
                flattened = xcen*dtc_shape[1] + ycen
